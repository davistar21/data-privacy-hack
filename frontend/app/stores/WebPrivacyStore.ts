...
import { websites as seed } from "../../constants/website";
import type {
  CookieAuditEvent,
  GlobalPreference,
  OrgAuditEvent,
...
export const useWebPrivacyStore = create<WebPrivacyState>()(
  persist(
    (set, get) => {
      const computeRiskScore = (site: CookieSite) => {
        ...
        const reputationFactor = Math.max(
          0,
          50 - Math.round(site.reputation / 2)
        );
        const catPenalty =
          ...
          set((s) => ({
            websites: s.websites.map((w) => {
              ...
              (Object.keys(categories) as CookieCategory[]).forEach((cat) => {
                if (categories[cat] === "allow_once")
                  categories[cat] = "disabled";
              });
              return {
                ...w,
                categories,
    ...
    {
      name: "web-privacy-store",
      partialize: (state) => ({
        websites: state.websites,
        globalPreference: state.globalPreference,
        cookieLogs: state.cookieLogs,
      }),
    }
  )
);
